...
from app.database import Base, LOCAL_USER_ID, engine, user_session
...
def test_scheduled_due_requires_successful_durable_run_after_daily_time(monkeypatch):
    _clear_scan_runs()
    tz = ZoneInfo("Asia/Jerusalem")
    monkeypatch.setattr(main.settings, "scan_hour", 8)
    monkeypatch.setattr(main.settings, "scan_minute", 0)
    monkeypatch.setattr(main.settings, "timezone", "Asia/Jerusalem")
    now_local = datetime(2026, 8, 12, 9, 0, tzinfo=tz)

    with user_session(LOCAL_USER_ID) as db:
        profile = main.get_user_profile(db)
        track = main.active_track(profile)
        failed, _ = create_scan_run(db, track, trigger="scheduled")
        _set_scan_finished_at(failed, status="failed", finished_at=datetime(2026, 8, 12, 8, 15, tzinfo=tz))
        db.commit()
        due, _scheduled, _latest = scheduled_scan_due(db, track, now_local=now_local)
        assert due is True

        successful, _ = create_scan_run(db, track, trigger="scheduled")
        # The failed run is finished, so a new run is allowed. Mark this one successful.
        _set_scan_finished_at(successful, status="ok", finished_at=datetime(2026, 8, 12, 8, 30, tzinfo=tz))
        db.commit()
