...
function trackingNavigatorMarkup(status=''){const {index,total}=trackedApplicationPosition(),known=index>=0;if(!known||!total)return '';const attemptCount=Number(applicationTrackingData?.application?.attempt_count||0),blockerKind=applicationTrackingData?.application?.blocker?.kind||'',blindRetryBlocked=['submit_not_sent','anti_automation_blocked'].includes(blockerKind),retryable=!blindRetryBlocked&&(['failed','needs_input','verification_pending'].includes(status)||(status==='queued'&&attemptCount>0));return `<nav class="application-tracker-navigator" aria-label="מעבר בין הגשות שלא הושלמו"><button type="button" onclick="moveTrackedApplication(-1)" aria-label="המשרה הקודמת" title="המשרה הקודמת">→</button><span><strong>משרה ${index+1} מתוך ${total}</strong><small>הגשות שטרם הושלמו</small></span>${retryable?`<button class="application-tracker-retry" type="button" onclick="retryTrackedApplication(${trackedApplicationId},this)" aria-label="הגשה מחדש" title="הגשה מחדש">↻</button>`:'<i></i>'}<button type="button" onclick="moveTrackedApplication(1)" aria-label="המשרה הבאה" title="המשרה הבאה">←</button></nav>`}
...
function applicationProgressMarkup(){
  ...
  const guidedFailureAction=failed&&!manualRequired&&['submit_not_sent','anti_automation_blocked'].includes(blocker?.kind)?`<div class="card-actions"><button class="btn primary small" type="button" onclick="openInteractiveBlockedApplication(${data.application.id},this)">פתח בדיקה מונחית</button></div>`:'';
