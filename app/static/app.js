...
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),running=queue.running||[],waiting=queue.waiting||[],attention=queue.attention||[],current=queue.current,total=Number(queue.total_active_count||running.length+waiting.length+attention.length),bulkRetryCount=attention.filter(item=>['needs_input','failed'].includes(item.status)).length;const runningMarkup=running.map((item,index)=>autoQueueRunningMarkup(item,index,running.length)).join('');const attentionMarkup=attention.map(autoQueueAttentionMarkup).join('');const queuedHead=!running.length&&current?.status==='queued'&&!waiting.some(item=>Number(item.id)===Number(current.id))?[current,...waiting]:waiting;const waitingMarkup=queuedHead.map((item,index)=>autoQueueWaitingMarkup(item,index,running.length)).join('');const emptyMarkup=!total&&!running.length&&!attention.length?emptyState('✓','התור ריק','אין כרגע משרות שממתינות להגשה אוטומטית.'):'';const runningSummary=running.length?`${running.length} ${running.length===1?'worker פעיל':'workers פעילים'} כרגע`:'אין worker פעיל כרגע';modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${total} ${total===1?'משרה פעילה':'משרות פעילות'} בתור ובטיפול</h2><p class="muted"><strong>${runningSummary}.</strong> ירוק מציין הגשה שרצה, צהוב מציין המתנה או שאלה שאפשר לענות עליה כאן, ואדום מציין שגיאה או הגשה שלא אומתה.</p><div class="auto-apply-queue-list" data-auto-queue-content="true">${runningMarkup}${attentionMarkup}${waitingMarkup}${emptyMarkup}</div><div class="modal-actions">${bulkRetryCount?`<button class="btn secondary auto-queue-retry-all" type="button" onclick="retryAllAutoQueueApplications(this)" title="הפעל מחדש את כל ההגשות התקועות"><span>↻</span> הפעל מחדש את כולן (${bulkRetryCount})</button>`:''}<button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`);bindChoiceBlockerButtons($('#modal-content'))}
