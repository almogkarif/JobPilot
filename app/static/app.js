...
function autoQueueRunningMarkup(item,index,total){const worker=String(item.agent_id||'').replace(/^github-actions-/,'GitHub Actions #');const duplicate=Number(item.duplicate_of||0);return `<article class="auto-queue-current is-active is-running ${duplicate?'is-duplicate':''}"><b>▶</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ${duplicate?`כפילות אפשרית של הגשה #${duplicate}`:`worker פעיל${worker?` · ${esc(worker)}`:''}`}</small></span><em>${duplicate?'בדיקת כפילות':total>1?`רץ עכשיו · ${index+1}/${total}`:'רץ עכשיו'}</em>${autoQueueRowActions(item,{isCurrent:true})}</article>`}
...
function autoQueueAttentionMarkup(item){const blocker=item.blocker||{},options=Array.isArray(blocker.options)?blocker.options.filter(Boolean):[],isQuestion=item.status==='needs_input'&&['choice_required','unknown_field','missing_profile_detail'].includes(blocker.kind),isChoice=isQuestion&&options.length>=2&&options.length<=6,tone=isQuestion?'question':'danger',headline=isQuestion?'מחכה לתשובה שלך':item.status==='verification_pending'?'השליחה טרם אומתה':'ההגשה נעצרה',question=blocker.question||blocker.field_label||'',detail=blocker.explanation||item.last_error||'נדרשת בדיקה לפני ניסיון נוסף',rediscover=blocker.legacy_choice_question?`<button class="btn secondary small" type="button" onclick="resolveBlockerAction(${blocker.id},'rediscover_question',${item.id})">השאלה לא מוצגת — זהה מחדש</button>`:'',interaction=isChoice?`<div class="auto-queue-inline-answer"><strong>${esc(question||'בחר תשובה')}</strong><div>${options.map(option=>`<button class="btn secondary small" type="button" data-choice-blocker="${blocker.id}" data-choice-application="${item.id}" data-choice-answer="${esc(option)}">${esc(option)}</button>`).join('')}${rediscover}</div></div>`:isQuestion?`<div class="auto-queue-inline-answer"><strong>${esc(question||'נדרשת תשובה')}</strong><div><input type="text" maxlength="255" data-text-blocker-input="${blocker.id}" value="${esc(applicationTextAnswerDrafts.get(Number(blocker.id))||'')}" placeholder="כתוב תשובה קצרה"><button class="btn primary small" type="button" data-text-blocker="${blocker.id}" data-text-application="${item.id}">שמור והמשך</button></div></div>`:'';return `<article class="auto-queue-attention tone-${tone}"><b>${isQuestion?'?':'!'}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ${esc(isQuestion?(question||detail):detail)}</small></span><em>${esc(headline)}</em>${interaction}${autoQueueRowActions(item)}</article>`}
...
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),running=queue.running||[],waiting=queue.waiting||[],attention=queue.attention||[],current=queue.current,total=Number(queue.total_active_count||running.length+waiting.length+attention.length),bulkRetryCount=attention.filter(item=>['needs_input','failed'].includes(item.status)).length;if(!current&&!running.length&&!waiting.length&&!attention.length){closeModal();toast('אין כרגע משרות בתור להגשה אוטומטית');return}const runningMarkup=running.map((item,index)=>autoQueueRunningMarkup(item,index,running.length)).join('');const attentionMarkup=attention.map(autoQueueAttentionMarkup).join('');const queuedHead=!running.length&&current?.status==='queued'&&!waiting.some(item=>Number(item.id)===Number(current.id))?[current,...waiting]:waiting;const waitingMarkup=queuedHead.map((item,index)=>autoQueueWaitingMarkup(item,index,running.length)).join('');const runningSummary=running.length?`${running.length} ${running.length===1?'worker פעיל':'workers פעילים'} כרגע`:'אין worker פעיל כרגע';modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${total} ${total===1?'משרה פעילה':'משרות פעילות'} בתור ובטיפול</h2><p class="muted"><strong>${runningSummary}.</strong> ירוק מציין הגשה שרצה, צהוב מציין המתנה או שאלה שאפשר לענות עליה כאן, ואדום מציין שגיאה או הגשה שלא אומתה.</p><div class="auto-apply-queue-list" data-auto-queue-content="true">${runningMarkup}${attentionMarkup}${waitingMarkup}</div><div class="modal-actions">${bulkRetryCount?`<button class="btn secondary auto-queue-retry-all" type="button" onclick="retryAllAutoQueueApplications(this)" title="הפעל מחדש את כל ההגשות התקועות"><span>↻</span> הפעל מחדש את כולן (${bulkRetryCount})</button>`:''}<button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`);bindChoiceBlockerButtons($('#modal-content'))}
...
function applicationProgressMarkup(){
  ...
  const choicePanel=choiceWaiting?`<div class="application-live-choice"><strong>${esc(blocker.question||'נדרשת בחירה כדי להמשיך')}</strong><div>${choiceOptions.map((option)=>`<button type="button" data-choice-blocker="${blocker.id}" data-choice-application="${data.application.id}" data-choice-answer="${esc(option)}">${esc(option)}</button>`).join('')}${blocker.legacy_choice_question?`<button type="button" onclick="resolveBlockerAction(${blocker.id},'rediscover_question',${data.application.id})">השאלה לא מוצגת — זהה מחדש</button>`:''}</div><small>בחר תשובה וה־Agent יחזור אוטומטית להגשה עם הבחירה הזו.</small></div>`:'';
