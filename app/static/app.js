const state = {
  ...
  sources: [],
  dashboard: null,
  autoApplyQueue: { current: null, running: [], running_count: 0, waiting: [], waiting_count: 0, queued_count: 0, total_active_count: 0 },
...
const SOURCE_LOGO_DOMAINS = Object.freeze({
  ...
  '4m analytics': '4manalytics.com',
  'exodigo': 'exodigo.com',
  'paragon': 'paragonsec.com',
  ...
  'fiverr': 'fiverr.com',
  'משרד הביטחון': 'mod.gov.il',
  'tower semiconductor': 'towersemi.com',
  'icl': 'icl-group.com',
  'teva': 'teva.com',
  'strauss group': 'strauss-group.com',
  'osem-nestle': 'osem-nestle.co.il',
  'החברה המרכזית למשקאות': 'cocacola.co.il',
  'fox group': 'fox.co.il',
  'shufersal': 'shufersal.co.il',
  'super-pharm': 'super-pharm.co.il',
  'deloitte israel': 'deloitte.com',
  'ey israel': 'ey.com',
  'pwc israel': 'pwc.com',
  'kpmg israel': 'kpmg.com',
  'tefen': 'tefen.com',
  'niram gitan': 'niramgitan.com',
  'ness': 'ness-tech.co.il',
  'matrix': 'matrix.co.il',
  'malam team': 'malamteam.com',
  'one technologies': 'one1.co.il',
  'elad systems': 'eladsoft.com',
  'israel post': 'israelpost.co.il',
  'ups': 'ups.com',
  'dhl': 'dhl.com',
  'israel railways': 'rail.co.il',
  'ashdod port': 'ashdodport.co.il',
  'haifa port': 'haifaport.co.il',
  'fridenson': 'fridenson.co.il',
  ...
  'the phoenix': 'fnx.co.il',
  'migdal': 'migdal.co.il',
  'clalit': 'clalit.co.il',
  'maccabi healthcare': 'maccabi4u.co.il',
  'sheba medical center': 'sheba.co.il',
  'ichilov medical center': 'tasmc.org.il',
});
...
const statusLabel = (status) => ({
  ...
  manual_required: 'הגשה ידנית', verification_pending: 'ממתין לאימות', submitted: 'הוגשה ואומתה', phone_screen: 'סינון טלפוני',
...
const blockerMeta = (kind) => ({
  ...
  anti_automation_blocked: { icon: '⛔', label: 'נדרשת הגשה ידנית', short: 'מערכת הגיוס חסמה את ההגשה האוטומטית', tone: 'danger' },
  security_code_required: { icon: '⌨', label: 'ממתין לקוד', short: 'הדבק את הקוד האחרון שנשלח למייל', tone: 'warning' },
...
function renderApplicationActions(application) {
  ...
  if (blocker?.kind === 'review_before_submit') {
    return `<button class="btn primary small" type="button" onclick="event.stopPropagation();retryAutomaticApplication(${application.id},{status:'failed',button:this})">הגש אוטומטית</button>
      <button class="btn secondary small" type="button" onclick="event.stopPropagation();openInteractiveBlockedApplication(${application.id},this)">פתח בדיקה מונחית</button>
  ...
  if (blocker?.kind === 'anti_automation_blocked') {
    return `<button class="btn primary small" type="button" onclick="event.stopPropagation();openInteractiveBlockedApplication(${application.id},this)">פתח בדיקה מונחית</button>
  ...
  if (blocker) {
    return `<a class="btn primary small" target="_blank" rel="noopener" href="${safeUrl(blockerTarget(blocker, application))}" onclick="event.stopPropagation()">פתח והמשך</a>
      ...
      ${blocker.kind === 'submit_not_sent'
        ? `<button class="btn secondary small" type="button" onclick="event.stopPropagation();openInteractiveBlockedApplication(${application.id},this)">פתח בדיקה מונחית</button>`
        : ['submit_button_missing','application_form_missing'].includes(blocker.kind)
  ...
  if (application.status === 'manual_required') {
    return `<a class="btn primary small" target="_blank" rel="noopener" href="${safeUrl(application.job?.apply_url||'#')}" onclick="event.stopPropagation()">פתח להגשה ידנית</a>
      <button class="btn secondary small" type="button" onclick="event.stopPropagation();markApplicationSubmitted(${application.id})">סמן כהוגש ידנית</button>`;
  }
...
function modal(html) {
  modal.descriptionObserver?.disconnect();
  ...
  const updateFilterShadows = () => filterDetails.forEach(detail => {
    detail.parentElement.classList.toggle('has-more-detail', detail.scrollHeight - detail.clientHeight - detail.scrollTop > 2);
  });
  modal.filterScrollObserver = new ResizeObserver(updateFilterShadows);
  filterDetails.forEach(detail => {
    detail.addEventListener('scroll', updateFilterShadows, { passive: true });
    modal.filterScrollObserver.observe(detail);
  });
  updateFilterShadows();
...
async function loadDashboard() {
  ...
  rankingStatus.hidden = !rankingIsLoading && !rankingFailed;
  rankingStatus.innerHTML = rankingFailed ? `
    <span><strong>${rankingRefresh.phase === 'failed' ? 'רענון הדירוג נכשל' : 'חלק מהמשרות לא דורגו'}</strong><small>${esc(rankingRefresh.message || 'הדירוגים התקינים נשמרו. אפשר לנסות שוב.')}</small></span>
  ...
  if (retryRanking) retryRanking.onclick = async () => {
    retryRanking.disabled = true;
    try {
      const query = rankingRefresh.phase === 'failed' ? '' : '?failed_only=true';
      await api('/api/ranking/refresh' + query, {method:'POST'});
      dashboardRankingRefreshPolls = 0;
      await loadDashboard();
    } catch (error) {
      toast(error.message);
      retryRanking.disabled = false;
    }
  };
  clearInterval(dashboardRankingCountdownTimer);
  dashboardRankingCountdownTimer=null;
  ...
  if (recommendationsPending && !rankingFailed && !rankingRefresh.running && !dashboardRankingRecoveryTracks.has(dashboard.career_track)) {
  ...
  renderScanSuggestions(dashboard.scan_suggestions || []);
...
function renderScanSuggestions(jobs) {
  const root = $('#scan-suggestions');
  root.innerHTML = jobs.length ? jobs.slice(0, 3).map((job) => `
    <article class="scan-suggestion-card">
      <div class="scan-suggestion-heading">
        ${sourceLogoMarkup({company_name:job.company,name:job.company}, 'scan-suggestion-logo')}
        <div class="scan-suggestion-title"><span class="scan-suggestion-company" dir="auto">${esc(job.company)}</span><h3 dir="auto">${esc(job.title)}</h3></div>
        <span class="scan-suggestion-score"><b>${Math.round(Number(job.score))}%</b><small>התאמה</small></span>
      </div>
      <div class="scan-suggestion-footer"><span class="scan-suggestion-location" dir="auto">${esc(job.location || 'המיקום לא צוין')}</span><button class="text-btn" type="button" data-suggestion-job="${Number(job.id)}" aria-label="${esc(`לפרטי המשרה ${job.title}`)}">לפרטי המשרה <span aria-hidden="true">←</span></button></div>
    </article>`).join('') : '<p class="scan-suggestions-empty">כשתימצאנה משרות נוספות מהסריקות האחרונות עם התאמה של 70% ומעלה, הן יופיעו כאן.</p>';
  $$('[data-suggestion-job]', root).forEach((button) => {
    button.onclick = () => showJob(Number(button.dataset.suggestionJob));
  });
}
...
async function loadJobs(options = {}) {
  ...
  const excludeSubmitted = !$('#job-show-submitted').checked;
  ...
  const payload = await api(`/api/jobs?min_score=${score}&status=${status}&${locationQuery ? `${locationQuery}&` : ''}${adminFilterQuery ? `${adminFilterQuery}&` : ''}query=${query}&paginated=true&page=${state.jobsPaging.page}&page_size=${pageSize}&sort=${encodeURIComponent(sort)}&automatic_only=${automaticOnly}&exclude_submitted=${excludeSubmitted}`);
...
$('#job-status-filter').onchange = () => {
  if ($('#job-status-filter').value === 'submitted') $('#job-show-submitted').checked = true;
  loadJobs({ resetPage: true });
};
$('#job-show-submitted').onchange = () => {
  if (!$('#job-show-submitted').checked && $('#job-status-filter').value === 'submitted') $('#job-status-filter').value = '';
  loadJobs({ resetPage: true });
};
...
function viewInteractiveApplication(applicationId) {
  ...
  liveWindow.document.title = 'JobPilot — מכין צפייה חיה';
  liveWindow.document.body.innerHTML = '<main dir="rtl" style="font-family:system-ui;text-align:center;padding:64px"><h1>מכין צפייה חיה…</h1><p>החלון יתחבר לסוכן בעוד מספר שניות.</p></main>';
  openInteractiveLiveView(Number(applicationId), liveWindow);
}
...
function rankingStatusMeta(status) {
  const map={
    ...
    preferred:['יתרון / התאמה חלקית','warn'],stretch:['גבולי','warn'],preference_mismatch:['מחוץ להעדפה','warn'],
...
function v2ExperienceDetail(e) {
  if (e.experience_preferred_only) return `ניסיון מצוין כיתרון בלבד, ולא כתנאי חובה${e.preferred_experience_evidence?` · ${e.preferred_experience_evidence}`:''}`;
...
function v2DegreeDetail(e) {
  if(e.track_degree_explanation)return e.track_degree_explanation;
...
function v2DeductionDetail(job,key,part) {
  ...
  if (key==='preferences') {
    ...
    if (e.work_mode_status==='unknown') reasons.push('מודל העבודה לא צוין במשרה');
    else if (e.work_mode_status!=='match') reasons.push('מודל העבודה אינו תואם להעדפות');
...
function v2WarningHebrew(value) {
  ...
  match=text.match(/^Unidentified job requirements: (degree, experience|degree|experience); penalty: (\d+) points$/);
  if(match) {
    const missing={degree:'דרישת תואר',experience:'דרישת ניסיון','degree, experience':'דרישת תואר ודרישת ניסיון'}[match[1]];
    return `${missing} לא זוהתה במודעה — הופחתו ${match[2]} נקודות`;
  }
...
async function copyJobId(id) {
  try {
    ...
    toast('מספר המשרה הועתק');
  } catch {
    toast('ההעתקה לא הצליחה. אפשר לסמן ולהעתיק את המספר ידנית.');
  }
}

function renderV2RankingExplanation(job) {
  const idCard=`<article class="ranking-filter neutral ranking-job-id"><span>מזהה משרה (ID)</span><strong class="job-id-value" dir="ltr">#${esc(job.id)}<button class="job-id-copy" type="button" aria-label="העתק מספר משרה" title="העתק מספר משרה" onclick="copyJobId(${Number(job.id)})"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="8" y="8" width="12" height="12" rx="2"/><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h3"/></svg></button></strong></article>`;
  if (job.ranking_pending || !job.eligibility) return `<section class="ranking-v2-explanation"><div class="ranking-eligibility-grid">${idCard}</div><div class="empty-state">הדירוג מתעדכן… המשרה נשארת זמינה בזמן החישוב.</div></section>`;
  ...
  const filterRows=[
    ...
    ['ניסיון',e.experience_status,v2ExperienceDetail(e),!e.experience_preferred_only&&(e.required_experience_min===null||e.required_experience_min===undefined)],
    ['תואר',({green:'match',yellow:'preferred',red:'mismatch'})[e.track_degree_color]||e.degree_status,v2DegreeDetail(e),!e.track_degree_color&&!e.required_degree],
  ...
  const filters=filterRows.map(([label,status,detail,detectionMissing])=>{const [statusLabel,baseTone]=rankingStatusMeta(status),tone=label==='מודל עבודה'&&status==='unknown'?'warn':baseTone;return `<article class="ranking-filter ${tone}${detectionMissing?' detection-missing':''}"><span>${esc(label)}</span><strong>${esc(statusLabel)}</strong><small tabindex="0" aria-label="${esc(`פירוט ${label}`)}">${esc(detail)}</small></article>`}).join('')+idCard;
  ...
  ].map(([label,key,detail])=>{const part=b[key]||{},score=Number(part.score)||0,max=Number(part.max)||0,pct=max?Math.max(0,Math.min(100,Math.round(score/max*100))):0,deducted=Math.max(0,max-score);return `<article class="ranking-score-card${key==='preferences'&&e.work_mode_status==='unknown'&&['match','unknown','not_configured'].includes(e.location_status)?' work-mode-unknown':''}"><header><span>${esc(label)}</span><strong>${score}/${max}</strong></header><i><b style="width:${pct}%"></b></i><small>${esc(detail)}</small>${deducted?`<em><b>ירדו ${deducted} נקודות:</b> ${esc(v2DeductionDetail(job,key,part))}</em>`:''}</article>`}).join('');
  const adjustments=[];
  ...
  try {
    ...
    modal(`
      ...
      <section class="job-description-section">
        ...
        <div class="job-description-scroll-wrap"><div class="job-description-scroll" tabindex="0" role="region" aria-label="תיאור המשרה">${formatJobDescription(job.description)}</div><div class="job-description-scroll-hint" hidden aria-hidden="true">גללו להמשך תיאור המשרה ↓</div></div>
      </section>
      <h3>אפשרויות הגשה</h3>
    ...
    `);
    const descriptionScroll = $('.job-description-scroll', $('#modal-content'));
    const descriptionHint = $('.job-description-scroll-hint', $('#modal-content'));
    const updateDescriptionHint = () => {
      descriptionHint.hidden = descriptionScroll.scrollHeight - descriptionScroll.clientHeight - descriptionScroll.scrollTop <= 2;
    };
    descriptionScroll.addEventListener('scroll', updateDescriptionHint, { passive: true });
    modal.descriptionObserver = new ResizeObserver(updateDescriptionHint);
    modal.descriptionObserver.observe(descriptionScroll);
    if (descriptionScroll.firstElementChild) modal.descriptionObserver.observe(descriptionScroll.firstElementChild);
...
let applicationsRefreshBusy = false;

function stopApplicationsRefresh() {
  if (applicationsRefreshTimer) clearTimeout(applicationsRefreshTimer);
  applicationsRefreshTimer = null;
}

function scheduleApplicationsRefresh() {
  stopApplicationsRefresh();
  if (state.activeView !== 'applications' || state.applicationSection !== 'queue') return;
  applicationsRefreshTimer = setTimeout(async () => {
    if (applicationsRefreshBusy || state.activeView !== 'applications' || state.applicationSection !== 'queue') return;
    applicationsRefreshBusy = true;
    try { await loadApplications({ silent: true }); }
    catch (error) { console.warn('Applications live refresh failed', error); }
    finally { applicationsRefreshBusy = false; }
  }, document.visibilityState === 'hidden' ? 20000 : 5000);
}
...
async function loadApplications({ silent = false } = {}) {
  ...
  if (!silent) root.innerHTML = skeleton(4, 'rows');
  try {
    ...
    <table><thead><tr><th>#</th><th>פעולה</th><th>משרה</th><th>חברה</th><th>סטטוס</th><th>מצב</th><th>ניסיונות</th><th>עודכן</th></tr></thead>
    <tbody>${state.applications.map((application, index) => `
      <tr class="interactive-table-row" data-job-id="${application.job_id}" tabindex="0">
        <td><span class="application-list-number">${index + 1}</span></td>
...
function applicationBoardStatus(application){
  if(application.status==='queued'&&application.auto_queue_eligible!==true)return 'saved';
  if(['manual_required','cancelled','skipped'].includes(application.status))return 'saved';
  if(['needs_input','failed','verification_pending'].includes(application.status))return 'attention';
  if(['phone_screen','test','interview'].includes(application.status))return 'interview';
  if(['offer','accepted'].includes(application.status))return 'offer';
  return ['saved','queued','applying','submitted','rejected'].includes(application.status)?application.status:'saved';
}
function renderApplicationsKanban(root) {
  const columns = [['saved','נשמרה / ידנית','saved'],['queued','בתור אוטומטי','queued'],['applying','רץ עכשיו','applying'],['attention','דורש טיפול','needs_input'],['submitted','הוגשה','submitted'],['interview','ראיון','interview'],['offer','הצעה','offer'],['rejected','נדחתה','rejected']];
  root.classList.add('kanban-wrap');
  const listNumber = new Map(state.applications.map((item,index)=>[Number(item.id),index+1]));
  root.innerHTML = state.applications.length ? `<div class="kanban-board">${columns.map(([status,label,dropStatus]) => `<section class="kanban-column" data-status="${dropStatus}"><header><strong>${label}</strong><span>${state.applications.filter(a=>applicationBoardStatus(a)===status).length}</span></header><div>${state.applications.filter(a=>applicationBoardStatus(a)===status).map(a=>`<article class="kanban-card status-${a.status}" draggable="true" data-application-id="${a.id}" onclick="showJob(${a.job_id})"><b class="application-list-number">${listNumber.get(Number(a.id))}</b><strong>${esc(a.job?.title)}</strong><span>${esc(a.job?.company)}</span><small class="kanban-status">${esc(statusLabel(a.status))}${a.status==='queued'&&a.auto_queue_eligible===true&&a.queue_position?` · מקום ${a.queue_position} בתור`:''}</small>${a.status==='queued'&&a.auto_queue_eligible!==true?'<small class="kanban-warning">לא ממתינה ל־Auto Apply</small>':''}${a.reminder_at ? `<small>תזכורת: ${dateFmt(a.reminder_at)}</small>`:''}<button type="button" onclick="event.stopPropagation();editApplication(${a.id})">ניהול</button></article>`).join('')}</div></section>`).join('')}</div>` : emptyState('↗','עדיין אין הגשות','משרות שתשמור או תוסיף לתור יופיעו כאן.');
...
function renderBlockerCard(blocker) {
  ...
  if (blocker.kind === 'review_before_submit') {
    ...
    interaction = `<div class="blocker-manual-note"><strong>מערכת הגיוס דורשת אישור אנושי.</strong><br>אפשר לפתוח בדיקה מונחית שתמלא את הטופס ותעצור לפני השליחה.</div><div class="blocker-decision"><button class="btn primary" type="button" onclick="openInteractiveBlockedApplication(${blocker.application_id},this)">פתח בדיקה מונחית</button></div>`;
  } else if (blocker.kind === 'submit_not_sent') {
    interaction = `<div class="blocker-manual-note">לא זוהתה בקשת הגשה שיצאה מהדפדפן. אפשר לפתוח בדיקה מונחית כדי לראות את החסימה לפני ניסיון שליחה נוסף.</div><div class="blocker-decision"><button class="btn primary" type="button" onclick="openInteractiveBlockedApplication(${blocker.application_id},this)">פתח בדיקה מונחית</button></div>`;
  ...
  } else if (blockerChoiceOptions(blocker).length) {
    interaction = `${blockerChoiceControlMarkup(blocker, blocker.application_id)}
      ${blocker.legacy_choice_question?`<button class="btn secondary small" type="button" onclick="resolveBlockerAction(${blocker.id},'rediscover_question',${blocker.application_id})">השאלה לא מוצגת — זהה מחדש</button>`:''}
...
const CHOICE_BLOCKER_KINDS=new Set(['choice_required','unknown_field','missing_profile_detail']);
function blockerChoiceOptions(blocker={}){return CHOICE_BLOCKER_KINDS.has(blocker?.kind)&&Array.isArray(blocker?.options)?blocker.options.map(value=>String(value||'').trim()).filter(Boolean):[]}
function blockerChoiceControlMarkup(blocker={},applicationId=0){const options=blockerChoiceOptions(blocker),blockerId=Number(blocker?.id||0),appId=Number(applicationId||blocker?.application_id||0);if(!options.length)return '';if(options.length<=8)return `<div class="blocker-choice-options" aria-label="אפשרויות תשובה">${options.map(option=>`<button class="btn secondary small" type="button" data-choice-blocker="${blockerId}" data-choice-application="${appId}" data-choice-answer="${esc(option)}">${esc(option)}</button>`).join('')}</div>`;return `<div class="blocker-choice-select"><select data-choice-select="${blockerId}" aria-label="בחר תשובה"><option value="">בחר תשובה…</option>${options.map(option=>`<option value="${esc(option)}">${esc(option)}</option>`).join('')}</select><button class="btn primary small" type="button" data-choice-select-submit="${blockerId}" data-choice-application="${appId}">שמור והמשך</button></div>`}
...
function bindChoiceBlockerButtons(root) {
  ...
  $$('[data-choice-select-submit]', root).forEach((button) => {
    const blockerId=Number(button.dataset.choiceSelectSubmit),applicationId=Number(button.dataset.choiceApplication);
    const select=root.querySelector(`[data-choice-select="${blockerId}"]`);
    button.onclick=()=>{if(!select?.value)return toast('צריך לבחור תשובה');resolveChoiceBlocker(blockerId,applicationId,select.value,button)};
  });
...
async function resolveTextBlocker(blockerId, applicationId, input, button = null) {
  const answer=input?.value.trim()||'';
  if(!answer)return toast('צריך להזין תשובה קצרה');
  const originalText=button?.textContent||'';
  if(button){button.disabled=true;button.textContent='ממשיך…'}
  try{
    ...
    await Promise.all([loadBlockers(),loadApplications(),loadDashboard()]);
    if(Number(applicationId)===Number(trackedApplicationId))startApplicationTracking(applicationId,false);
  }catch(error){if(button){button.disabled=false;button.textContent=originalText}toast(error.message)}
}
...
async function resolveBlockerAction(id, action, applicationId = null) {
  try {
    ...
    else if (action === 'rediscover_question') toast('השאלה נשלחה לזיהוי מחדש — אין צורך לנחש תשובה');
    else toast('ההגשה דולגה');
    if (['use_profile_grade_sheet','rediscover_question'].includes(action) && applicationId) startApplicationTracking(applicationId, true);
    await Promise.all([loadBlockers(), loadApplications(), loadDashboard()]);
  } catch (error) {
    toast(error.message);
  }
...
const PROFILE_TEXT_FIELDS = [
  ...
  'preferred_work_modes', 'seniority_levels', 'keywords', 'excluded_keywords', 'auto_apply_threshold',
];
...
const PROFILE_ARRAY_FIELDS = new Set(['seniority_levels', 'years_experience_options', 'skills', 'desired_titles', 'preferred_locations', 'preferred_work_modes', 'keywords', 'excluded_keywords']);
...
const SEARCH_PREFERENCE_FIELDS = new Set(['seniority_levels', 'skills', 'desired_titles', 'preferred_locations', 'preferred_work_modes', 'keywords', 'excluded_keywords']);
const PROFILE_NUMBER_FIELDS = new Set(['auto_apply_threshold']);
// Location is a legacy hidden mirror of extra_city, maintained by the server.
const PROFILE_FIELDS = [...PROFILE_TEXT_FIELDS.filter(name => name !== 'location'), ...PROFILE_CHECK_FIELDS, ...EXTRA_PROFILE_FIELDS];
...
function renderCitizenshipOptions(selected = ['Citizen (Israel)']) {
  ...
  control.tabIndex = -1;
  control.parentElement?.querySelectorAll('.citizenship-picker, .citizenship-option-grid').forEach(element => element.remove());
...
function employmentEntry(item = {}, index = 0) {
  ...
  return `<article class="employment-entry" data-employment-entry>
    ...
    <div class="form-grid">
      ...
      <label>תאריך התחלה<input data-work-field="start_date" type="text" dir="ltr" placeholder="YYYY או YYYY-MM" pattern="[0-9]{4}(-(?:0[1-9]|1[0-2]))?" value="${esc(item.start_date || '')}" /></label>
      <label>תאריך סיום<input data-work-field="end_date" type="text" dir="ltr" placeholder="YYYY או YYYY-MM" pattern="[0-9]{4}(-(?:0[1-9]|1[0-2]))?" value="${esc(item.end_date || '')}" /><small>השאר ריק אם זו העבודה הנוכחית.</small></label>
...
function profileFieldLabel(name) {
  const explicit = {
    ...
    seniority_levels:'רמות ניסיון להצגה', keywords:'מילות מפתח לקידום בדירוג', excluded_keywords:'מילים להחרגה מכותרת המשרה', auto_apply_threshold:'סף התאמה',
...
async function focusProfileField(name = '') {
  if (name === 'location') name = 'extra_city';
...
const PROFILE_COMPLETION_FIELDS = [
  ['full_name','שם מלא'], ['email','אימייל'], ['phone','טלפון'],
  ['linkedin_url','LinkedIn'], ['extra_city','עיר'], ['extra_education_school','מוסד לימודים'],
  ['degree_level','השכלה'], ['extra_languages','שפות'],
];
const PROFILE_CRITICAL_FIELDS = new Set(['full_name', 'email', 'phone', 'extra_city']);
...
async function loadResumeInsights(){
  ...
  const warnings=resumes.filter(resume=>resume.analysis?.warning||resume.analysis?.error||!resume.analysis?.text_length).map(resume=>`<p class="warning">${esc(resume.filename)}: לא ניתן היה להשלים את קריאת הקובץ. יש להעלות קובץ עם טקסט קריא כדי למלא פרטים אוטומטית.</p>`).join('');
  root.innerHTML=`<div class="resume-insights-head"><strong>${resumes.length} גרסאות קורות חיים</strong><button class="text-btn" type="button" data-resume-manager>ניהול גרסאות</button></div>${suggestions.length?`<div class="resume-suggestions"><span>סקילים ופרטים שסותרים מידע קיים מחכים לאישור שלך</span>${suggestions.map(item=>`<button type="button" data-resume-suggestion data-resume-id="${item.resume_id}" data-field="${esc(item.field)}" data-value="${encodeURIComponent(String(item.value))}"><b>＋</b>${esc(item.label)}<small>${esc(item.resume_label)}</small></button>`).join('')}</div>`:`<p class="resume-analysis-ok">הניתוח הסתיים. אין כרגע הצעות שממתינות לאישור; מומלץ לבדוק אילו פרטים זוהו בפרופיל.</p>`}`;
  if(warnings) { root.querySelector('.resume-analysis-ok')?.remove(); root.insertAdjacentHTML('beforeend',warnings); }
...
document.addEventListener('visibilitychange', () => {
  ...
  if (trackedApplicationId) {
    const status=applicationTrackingData?.application?.status||'';
    if (['queued','applying'].includes(status)) scheduleApplicationTrackingPoll(status);
  }
});
...
function trackingNavigatorMarkup(status=''){const {index,total}=trackedApplicationPosition(),known=index>=0;if(!known||!total)return '';const attemptCount=Number(applicationTrackingData?.application?.attempt_count||0),blockerKind=applicationTrackingData?.application?.blocker?.kind||'',blindRetryBlocked=['submit_not_sent','anti_automation_blocked','review_before_submit'].includes(blockerKind),retryable=!blindRetryBlocked&&(['failed','needs_input','verification_pending'].includes(status)||(status==='queued'&&attemptCount>0));return `<nav class="application-tracker-navigator" aria-label="מעבר בין הגשות שלא הושלמו"><button type="button" onclick="moveTrackedApplication(-1)" aria-label="המשרה הקודמת" title="המשרה הקודמת">→</button><span><strong>משרה ${index+1} מתוך ${total}</strong><small>הגשות שטרם הושלמו</small></span>${retryable?`<button class="application-tracker-retry" type="button" onclick="retryTrackedApplication(${trackedApplicationId},this)" aria-label="הגשה מחדש" title="הגשה מחדש">↻</button>`:'<i></i>'}<button type="button" onclick="moveTrackedApplication(1)" aria-label="המשרה הבאה" title="המשרה הבאה">←</button></nav>`}
...
function combinedApplicationQueue(snapshot=state.autoApplyQueue,tracked=trackingApplications){const queue=normalizeAutoApplyQueue(snapshot),seen=new Set(),unique=(items)=>items.filter(item=>{const id=Number(item?.id||0);if(!id||seen.has(id))return false;seen.add(id);return true}),running=unique([...(queue.running||[]),...(tracked||[]).filter(item=>item.status==='applying')]),attention=unique([...(queue.attention||[]),...(tracked||[]).filter(item=>['needs_input','manual_required','verification_pending','failed'].includes(item.status))]),waiting=unique([...(queue.waiting||[]),...(tracked||[]).filter(item=>item.status==='queued')]);if(queue.current&&!seen.has(Number(queue.current.id))){const target=queue.current.status==='applying'?running:queue.current.status==='queued'?waiting:attention;target.unshift(queue.current);seen.add(Number(queue.current.id))}return {...queue,running,attention,waiting,total_active_count:seen.size}}
...
function autoQueueRowActions(item,{isCurrent=false}={}){const automatic=item.mode!=='audit',running=item.status==='applying',queued=item.status==='queued',alreadyNext=isCurrent&&!running,retryable=automatic&&['needs_input','failed','verification_pending'].includes(item.status);return `<div class="auto-queue-row-actions"><button class="btn secondary small" type="button" onclick="openAutoQueueApplication(${item.id})">פתח</button>${retryable?`<button class="application-tracker-retry auto-queue-retry" type="button" onclick="retryAutomaticApplication(${item.id},{status:'${item.status}',button:this,refreshQueue:true})" aria-label="הגשה מחדש" title="הגשה מחדש">↻</button>`:''}${queued&&automatic?`<button class="btn secondary small" type="button" onclick="prioritizeAutoQueueApplication(${item.id})" ${running||alreadyNext?'disabled':''}>${alreadyNext?'הבא בתור':'הגש הבא בתור'}</button>`:''}<button class="btn danger-outline small" type="button" onclick="cancelAutoQueueApplication(${item.id})" ${running?'disabled title="לא ניתן לבטל worker שכבר רץ"':''}>ביטול</button></div>`}
...
function applicationProgressMarkup(){
  ...
  const choiceOptions=blockerChoiceOptions(blocker),choiceWaiting=status==='needs_input'&&choiceOptions.length>0;
  const textWaiting=status==='needs_input'&&['unknown_field','missing_profile_detail'].includes(blocker?.kind)&&!choiceOptions.length;
  const securityCodeWaiting=status==='applying'&&blocker?.kind==='security_code_required',gradeSheetWaiting=status==='needs_input'&&blocker?.kind==='grade_sheet_required',hasGradeSheet=Boolean(state.profile?.grade_sheet_uploaded),gradeSheetNeedsUpload=gradeSheetWaiting&&!hasGradeSheet,attentionWaiting=choiceWaiting||textWaiting||gradeSheetNeedsUpload||securityCodeWaiting;
  const verificationPending=status==='verification_pending',manualRequired=status==='manual_required'||blocker?.kind==='anti_automation_blocked',isRunning=status==='applying',queueSnapshot=normalizeAutoApplyQueue(data.auto_apply_queue||state.autoApplyQueue),runningQueueItem=queueSnapshot.running.find(item=>Number(item.id)===Number(data.application?.id)),duplicateRunning=isRunning&&Number(runningQueueItem?.duplicate_of||0)>0,failed=(['failed','needs_input'].includes(status)&&!attentionWaiting)||manualRequired,verified=status==='submitted'&&types.has('submission_verified');let firstPending=true;
  const rows=APPLICATION_PROGRESS_STEPS.map(([key,title,copy],index)=>{const event=attemptEvents.find(item=>item.event_type===key),done=queuedWaiting?(key==='queued'||(key==='worker_dispatched'&&types.has(key))):(types.has(key)||(key==='submission_verified'&&verified)),current=!done&&firstPending;if(current)firstPending=false;const stateClass=done?'done':current?(attentionWaiting?'choice-waiting':verificationPending&&key==='submission_verified'?'verification-waiting':failed?'failed':'active'):'pending';const marker=done?'✓':current&&choiceWaiting?'?':current&&gradeSheetNeedsUpload?'↑':current&&verificationPending&&key==='submission_verified'?'…':current&&failed?'!':index+1;const rowCopy=current&&queuedWaiting&&key==='attempt_started'?(queuePosition>1?`מיקום ${queuePosition} בתור · תופעל אוטומטית ברצף אחרי ההגשות שלפניה`:'ממתינה ל־worker · תתחיל אוטומטית כשהוא יתפנה'):current&&manualRequired?'מערכת הגיוס חסמה את האוטומציה. JobPilot לא יבצע ניסיון אוטומטי נוסף.':current&&gradeSheetWaiting?(hasGradeSheet?'הצירוף האוטומטי של גיליון הציונים נכשל':'חסר גיליון ציונים בפרופיל'):current&&verificationPending&&key==='submission_verified'?'נצפתה בקשת Submit; ממתינים לראיית אישור חד־משמעית מהאתר או ממייל':done?(event?.message||copy):copy;return `<li class="${stateClass}"><i>${marker}</i><span><strong>${esc(title)}</strong><small>${esc(rowCopy)}</small></span></li>`}).join('');
  const choicePanel=choiceWaiting?`<div class="application-live-choice"><strong>${esc(blocker.question||'נדרשת בחירה כדי להמשיך')}</strong>${blockerChoiceControlMarkup(blocker,data.application.id)}${blocker.legacy_choice_question?`<button type="button" onclick="resolveBlockerAction(${blocker.id},'rediscover_question',${data.application.id})">השאלה לא מוצגת — זהה מחדש</button>`:''}<small>בחר תשובה וה־Agent יחזור אוטומטית להגשה עם הבחירה הזו.</small></div>`:'';
  ...
  const securityCodePanel=securityCodeWaiting?`<div class="application-live-choice application-live-security-code">${securityCodeInputMarkup(data.application.id)}</div>`:'';
...
async function syncPrimaryApplicationTracking(newApplicationId,autoOpen=false){if(!applicationsWorkspaceAllowed()){if(newApplicationId)startApplicationTracking(Number(newApplicationId),false,true);if(autoOpen)openNotifications();return}const queue=await refreshAutoApplyQueue();const requestedId=Number(newApplicationId||0);let trackedStatus=applicationTrackingData?.application?.status||'';if(trackedApplicationId&&!applicationTrackingData){try{const tracking=await api(`/api/applications/${trackedApplicationId}/tracking-status`);if(tracking.auto_apply_queue)setAutoApplyQueue(tracking.auto_apply_queue);trackedStatus=tracking.status||''}catch{trackedStatus=''}}const preserveCurrent=Boolean(trackedApplicationId&&trackedStatus==='applying'&&!requestedId);if(!preserveCurrent){const primaryId=Number(requestedId||queue.current?.id||0);if(primaryId)startApplicationTracking(primaryId,false,Boolean(requestedId))}else{renderNotificationCenter()}if(autoOpen)openNotifications()}
...
function renderNotificationCenter() {
  ...
  const items = notificationItems(),actionableItems=items.filter(item=>!item.queue||Number(item.count)>0);
...
$('#notification-trigger').onclick = () => {
  ...
  clearInterval(notificationTrackingRefreshTimer);notificationTrackingRefreshTimer=null;
  if(open){refreshTrackingApplications().then(renderNotificationCenter);notificationTrackingRefreshTimer=setInterval(()=>refreshTrackingApplications().then(renderNotificationCenter),5000)}else renderNotificationCenter();
...
const RESUME_AUTOFILL_LABELS = Object.freeze({
  work_experiences: 'ניסיון תעסוקתי',
  languages:'שפות ורמת שליטה', degree_level:'סוג התואר',
  education_school:'מוסד לימודים', education_field:'תחום לימודים', education_grade:'ציון', education_start_date:'תחילת לימודים', education_end_date:'סיום לימודים',
...
$('#resume-file').onchange = async (event) => {
  ...
  try {
    ...
    const parts=[result.analysis?.warning||result.analysis?.error||!result.analysis?.text_length?'הקובץ נשמר, אך לא ניתן היה להשלים את הקריאה האוטומטית':'קורות החיים הועלו ונותחו'];
...
$('#privacy-center').onclick=async()=>{
  ...
  $('#resume-version-form').onsubmit=async e=>{e.preventDefault();const body=new FormData(e.target);if(!body.get('is_default'))body.set('is_default','false');await api('/api/resumes',{method:'POST',body});toast('גרסת קורות החיים נוספה');state.profileLoaded=false;await loadProfile();closeModal();$('#privacy-center').click();};
};
...
function onboardingSchedulePreferences(delay=0){
  ...
  const run=()=>{onboardingState.saveTimer=null;const draft=onboardingCollectPreferences();onboardingPersistProfile(draft).then(saved=>{
    onboardingState.draft={desired_titles:[...(saved.desired_titles||[])],preferred_locations:[...(saved.preferred_locations||[])],years_experience_options:[...(saved.years_experience_options||[])],degree_level:saved.degree_level||'',preferred_work_modes:[...(saved.preferred_work_modes||[])],seniority_levels:[...(saved.seniority_levels||[])],keywords:[...(saved.keywords||[])],excluded_keywords:[...(saved.excluded_keywords||[])]};
  }).catch(()=>{});};
...
const onboardingSteps = ['track','resume','resume-review','skills','preferences','review','ranking'];
...
function onboardingResumeReview(profile){
  const extra=profile.application_profile||{};
  const filled=new Set(onboardingState.resume?.autofilled_fields||[]);
  const field=(key,label,value,group=key,type='text')=>`<label><span class="onboarding-field-heading">${esc(label)}${filled.has(group)?'<small class="onboarding-autofilled">מולא מהקו״ח</small>':''}</span><input data-ob-profile-field="${key}" type="${type}" value="${esc(value||'')}" /></label>`;
  const employment=normalizeWorkExperiences(extra);
  const languages=Array.isArray(extra.languages)?extra.languages:[];
  return `<span class="kicker">בדיקת הפרטים</span><h1 id="onboarding-title">זה מה שמילאנו עבורך</h1><p>בדוק את הפרטים לפני שממשיכים. אפשר לתקן כאן; שדות ריקים לא זוהו בוודאות. פרטים שנשמרו קודם מוצגים גם הם.</p><p class="preference-help">מומלץ להיכנס לפרופיל לאחר סיום ההיכרות ולענות על השאלות האישיות, כדי שהמגיש האוטומטי יוכל להשתמש בתשובות שלך למילוי טופסי המועמדות.</p>
    ${!onboardingState.resume&&!profile.cv_filename?'<p class="preference-help">לא הועלו קורות חיים. אפשר למלא פרטים כעת או להמשיך ולהשלים בהמשך.</p>':''}
    <form id="onboarding-profile-review" class="onboarding-preferences">
      <fieldset class="full"><legend>זהות ופרטי קשר</legend><div class="form-grid">
      ${[['full_name','שם מלא'],['email','אימייל'],['phone','טלפון'],['linkedin_url','LinkedIn'],['github_url','GitHub'],['portfolio_url','אתר אישי']].map(([key,label])=>field(key,label,profile[key],key,key==='email'?'email':'text')).join('')}
      ${field('city','עיר מגורים',extra.city||profile.location,'location')}</div></fieldset>
      <fieldset class="full"><legend>השכלה</legend><div class="form-grid">
      ${field('education_school','מוסד לימודים',extra.education_school)}
      <label><span class="onboarding-field-heading">סוג תואר</span><select data-ob-profile-field="degree_level"><option value="">לא זוהה</option>${[['bachelor','תואר ראשון (B.A. / B.Sc.)'],['master','תואר שני (M.A. / M.Sc.)'],['phd','דוקטורט (Ph.D.)']].map(([key,label])=>`<option value="${key}" ${(extra.degree_level||profile.degree_level)===key?'selected':''}>${label}</option>`).join('')}</select></label>
      ${[['education_field','תחום לימודים'],['education_grade','ציון / GPA'],['education_start_date','תחילת לימודים'],['education_end_date','סיום לימודים']].map(([key,label])=>field(key,label,extra[key])).join('')}
      </div></fieldset>
      <fieldset class="full"><legend>ניסיון תעסוקתי${filled.has('work_experiences')?' · מולא מהקו״ח':''}</legend>
      <div id="onboarding-work-review">${employment.length?employment.map(employmentEntry).join(''):'<p>לא זוהו תפקידים למילוי אוטומטי. אפשר להשלים בפרופיל בהמשך.</p>'}</div></fieldset>
      <fieldset class="full"><legend>שפות${filled.has('languages')?' · מולא מהקו״ח':''}</legend>
      ${languages.length?languages.map(item=>`<div class="form-grid" data-ob-language><label>שפה<input data-ob-language-name value="${esc(item.name||'')}" /></label><label>רמת שליטה<input data-ob-language-level value="${esc(item.proficiency||'')}" /></label></div>`).join(''):'<p>לא זוהו שפות למילוי אוטומטי.</p>'}</fieldset>
    </form>`;
}
async function onboardingSaveResumeReview(){
  const form=$('#onboarding-profile-review');
  if(!form)return;
  if(!form.reportValidity())throw new Error('יש לתקן את השדות המסומנים לפני ההמשך');
  const patch={},extra={};
  const top=new Set(['full_name','email','phone','linkedin_url','github_url','portfolio_url']);
  $$('[data-ob-profile-field]',form).forEach(input=>{
    (top.has(input.dataset.obProfileField)?patch:extra)[input.dataset.obProfileField]=input.value.trim();
  });
  extra.work_experiences=$$('[data-employment-entry]',form).map(entry=>{
    const item={};$$('[data-work-field]',entry).forEach(input=>{item[input.dataset.workField]=input.value.trim()});return item;
  });
  extra.languages=$$('[data-ob-language]',form).map(entry=>({name:$('[data-ob-language-name]',entry).value.trim(),proficiency:$('[data-ob-language-level]',entry).value.trim()})).filter(item=>item.name);
  patch.application_profile=extra;
  patch.degree_level=extra.degree_level;
...
function onboardingSetStep(index){
  ...
  if(step==='track'){
    ...
    const upload=$('.onboarding-upload',content);
    upload.ondragover=event=>{event.preventDefault();event.stopPropagation();if(!upload.classList.contains('uploading')){event.dataTransfer.dropEffect='copy';upload.classList.add('is-dragging')}};
    upload.ondragleave=event=>{if(!upload.contains(event.relatedTarget))upload.classList.remove('is-dragging')};
    upload.ondrop=event=>{
      event.preventDefault();event.stopPropagation();upload.classList.remove('is-dragging');
      if(upload.classList.contains('uploading'))return;
      const files=event.dataTransfer?.files;
      if(!files?.length)return;
      if(files.length!==1){toast('יש להעלות קובץ קורות חיים אחד בכל פעם');return}
      onboardingResume({target:$('#onboarding-resume-file')},files[0]);
    };
  }else if(step==='resume-review'){
    content.innerHTML=onboardingResumeReview(profile);
    $('#onboarding-profile-review').onsubmit=event=>event.preventDefault();
    $$('[data-remove-employment]',content).forEach(button=>button.onclick=()=>button.closest('[data-employment-entry]').remove());
  }else if(step==='skills'){
    ...
    content.innerHTML=`<span class="kicker">העדפות · ${esc(track.label)}</span><h1 id="onboarding-title">נחדד את החיפוש</h1><p>בחר בכמה לחיצות את מה שמתאים לך. אפשר לדייק הכול גם אחר כך בהעדפות החיפוש.</p><div class="onboarding-form polished choice-form">
      ...
      <label><span>שנות הניסיון שלך</span><select id="ob-experience">${['0','1','2','3','4','5+'].map(v=>`<option ${(profile.years_experience_options||['0']).includes(v)?'selected':''}>${v}</option>`).join('')}</select></label>
      ...
      <fieldset class="onboarding-choice-field full"><legend>רמות ניסיון להצגה</legend><p>יוצגו רק משרות ברמות המסומנות, לאחר סינון ולפני דירוג. ללא סימון לא יוצגו משרות.</p><div class="onboarding-choice-grid compact">${experienceChoices.map(([v,l])=>onboardingChoiceBox('seniority',v,l,seniority.has(v))).join('')}</div></fieldset>
      <label class="onboarding-other full"><span>מילות מפתח לקידום בדירוג</span><input id="ob-keywords-extra" value="${esc((draft.keywords||profile.keywords||[]).join(', '))}" placeholder="למשל: infrastructure, developer tools"></label>
      <label class="onboarding-other full"><span>מילים להחרגה מכותרת המשרה</span><input id="ob-excluded-extra" value="${esc((draft.excluded_keywords||profile.excluded_keywords||[]).join(', '))}" placeholder="למשל: sales, manual QA"></label>
    </div>`;
...
async function onboardingResume(event,droppedFile=null){
  ...
  if(event.target.closest('.onboarding-upload')?.classList.contains('uploading'))return;
  ...
  if(!file.size||file.size>10*1024*1024){toast('יש להעלות קובץ שאינו ריק ועד 10MB');event.target.value='';return}
  const label=event.target.closest('.onboarding-upload'); label?.classList.add('uploading');
  const navigation=[$('#onboarding-next'),$('#onboarding-back'),$('#onboarding-skip')];navigation.forEach(button=>button.disabled=true);
...
async function saveOnboardingPreferences(){
  ...
  onboardingState.draft={
    ...
    seniority_levels:[...(state.profile.seniority_levels||[])],
...
function onboardingCollectPreferences(){
  ...
  onboardingState.draft={desired_titles:[...new Set([...selected('title'),...onboardingSplit($('#ob-titles-extra')?.value||'')])],preferred_locations:selected('location'),years_experience_options:[$('#ob-experience')?.value||'0'],degree_level:$('#ob-degree')?.value||'',preferred_work_modes:$$('[data-ob-mode].selected').map(b=>b.dataset.obMode),seniority_levels:selected('seniority'),keywords:[...new Set(onboardingSplit($('#ob-keywords-extra')?.value||''))],excluded_keywords:[...new Set(onboardingSplit($('#ob-excluded-extra')?.value||''))]};return onboardingState.draft;
}
...
function renderOnboardingRankingStatus(status){
  ...
  if(failed){
    ...
    target.innerHTML=`<span><b>רענון הדירוג לא הושלם</b><small>${esc(status.message||'אפשר לנסות שוב; הדירוגים התקינים נשמרו.')}</small></span><button type="button" class="btn secondary small" id="onboarding-retry-ranking">נסה שוב</button>`;
    $('#onboarding-retry-ranking').onclick=async(event)=>{
      const button=event.currentTarget;
      button.disabled=true;
      try{
        await api('/api/ranking/refresh'+(status.phase==='failed'?'':'?failed_only=true'),{method:'POST'});
...
async function onboardingWatchRanking(){
  try{
    ...
    if(!status.running && (Number(status.failed)>0 || status.phase==='failed'))return;
