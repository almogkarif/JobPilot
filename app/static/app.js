...
async function retryTrackedApplication(id){try{const uncertain=applicationTrackingData?.application?.status==='verification_pending';if(uncertain&&!confirm('לא התקבל אצלך אישור מהחברה? ניסיון חוזר עלול ליצור הגשה כפולה אם המועמדות כן נקלטה. להמשיך?'))return;const suffix=uncertain?'&confirm_not_submitted=true':'';await api(`/api/applications/${id}/retry?auto_submit=true${suffix}`,{method:'POST'});toast('ההגשה הוחזרה לתור ומופעלת מחדש');await refreshTrackingApplications();startApplicationTracking(id,false,true)}catch(error){toast(error.message)}}
...
function autoQueueAttentionMarkup(item){const blocker=item.blocker||{},options=Array.isArray(blocker.options)?blocker.options.filter(Boolean):[],isQuestion=item.status==='needs_input'&&['choice_required','unknown_field','missing_profile_detail'].includes(blocker.kind),isChoice=isQuestion&&options.length>=2&&options.length<=6,tone=isQuestion?'question':'danger',headline=isQuestion?'מחכה לתשובה שלך':item.status==='verification_pending'?'השליחה טרם אומתה':'ההגשה נעצרה',question=blocker.question||blocker.field_label||'',detail=blocker.explanation||item.last_error||'נדרשת בדיקה לפני ניסיון נוסף',interaction=isChoice?`<div class="auto-queue-inline-answer"><strong>${esc(question||'בחר תשובה')}</strong><div>${options.map(option=>`<button class="btn secondary small" type="button" data-choice-blocker="${blocker.id}" data-choice-application="${item.id}" data-choice-answer="${esc(option)}">${esc(option)}</button>`).join('')}</div></div>`:isQuestion?`<div class="auto-queue-inline-answer"><strong>${esc(question||'נדרשת תשובה')}</strong><div><input type="text" maxlength="255" data-text-blocker-input="${blocker.id}" value="${esc(applicationTextAnswerDrafts.get(Number(blocker.id))||'')}" placeholder="כתוב תשובה קצרה"><button class="btn primary small" type="button" data-text-blocker="${blocker.id}" data-text-application="${item.id}">שמור והמשך</button></div></div>`:'';return `<article class="auto-queue-attention tone-${tone}"><b>${isQuestion?'?':'!'}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ${esc(isQuestion?(question||detail):detail)}</small></span><em>${esc(headline)}</em>${interaction}${autoQueueRowActions(item)}</article>`}
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),running=queue.running||[],waiting=queue.waiting||[],attention=queue.attention||[],current=queue.current,total=Number(queue.total_active_count||running.length+waiting.length+attention.length);if(!current&&!running.length&&!waiting.length&&!attention.length){closeModal();toast('אין כרגע משרות בתור להגשה אוטומטית');return}const runningMarkup=running.map((item,index)=>autoQueueRunningMarkup(item,index,running.length)).join('');const attentionMarkup=attention.map(autoQueueAttentionMarkup).join('');const queuedHead=!running.length&&current?.status==='queued'&&!waiting.some(item=>Number(item.id)===Number(current.id))?[current,...waiting]:waiting;const waitingMarkup=queuedHead.map((item,index)=>autoQueueWaitingMarkup(item,index,running.length)).join('');const runningSummary=running.length?`${running.length} ${running.length===1?'worker פעיל':'workers פעילים'} כרגע`:'אין worker פעיל כרגע';modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${total} ${total===1?'משרה פעילה':'משרות פעילות'} בתור ובטיפול</h2><p class="muted"><strong>${runningSummary}.</strong> ירוק מציין הגשה שרצה, צהוב מציין המתנה או שאלה שאפשר לענות עליה כאן, ואדום מציין שגיאה או הגשה שלא אומתה.</p><div class="auto-apply-queue-list" data-auto-queue-content="true">${runningMarkup}${attentionMarkup}${waitingMarkup}</div><div class="modal-actions"><button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`);bindChoiceBlockerButtons($('#modal-content'))}
