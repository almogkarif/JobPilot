...
function trackingNavigatorMarkup(status=''){const {index,total}=trackedApplicationPosition(),known=index>=0;if(!known||!total)return '';const attemptCount=Number(applicationTrackingData?.application?.attempt_count||0),retryable=['failed','needs_input'].includes(status)||(status==='queued'&&attemptCount>0);return `<nav class="application-tracker-navigator" aria-label="מעבר בין הגשות שלא הושלמו"><button type="button" onclick="moveTrackedApplication(-1)" aria-label="המשרה הקודמת" title="המשרה הקודמת">→</button><span><strong>משרה ${index+1} מתוך ${total}</strong><small>הגשות שטרם הושלמו</small></span>${retryable?`<button class="application-tracker-retry" type="button" onclick="retryTrackedApplication(${trackedApplicationId})" aria-label="הגשה מחדש" title="הגשה מחדש">↻</button>`:'<i></i>'}<button type="button" onclick="moveTrackedApplication(1)" aria-label="המשרה הבאה" title="המשרה הבאה">←</button></nav>`}
...
async function retryTrackedApplication(id){try{await api(`/api/applications/${id}/retry?auto_submit=true`,{method:'POST'});toast('ההגשה הוחזרה לתור ומופעלת מחדש');await refreshTrackingApplications();startApplicationTracking(id,false,true)}catch(error){toast(error.message)}}
...
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),current=queue.current,waiting=queue.waiting||[],total=Number(queue.total_active_count||((current?1:0)+waiting.length));if(!current&&!waiting.length){closeModal();toast('אין כרגע משרות בתור להגשה אוטומטית');return}const currentRunning=current?.status==='applying',currentMarkup=current?`<article class="auto-queue-current is-active ${currentRunning?'is-running':'is-preparing'}"><b>${currentRunning?'▶':'…'}</b><span><strong>${esc(current.job?.title||'משרה')}</strong><small>${esc(current.job?.company||'')} · ${currentRunning?'רץ עכשיו':'בהכנה עכשיו'}</small></span><em>${currentRunning?'רץ עכשיו':'פעילה עכשיו'}</em>${autoQueueRowActions(current,{isCurrent:true})}</article>`:'';const waitingMarkup=waiting.map((item,index)=>`<article class="auto-queue-waiting"><b>${Number(item.queue_position||index+1)}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ממתינה בתור</small></span><em>ממתינה</em>${autoQueueRowActions(item)}</article>`).join('');modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${total} ${total===1?'משרה פעילה':'משרות פעילות'} בתור</h2><p class="muted">המשרה הירוקה היא הפעילה כעת: תחילה סביבת ה־worker מוכנה, ולאחר מכן היא מסומנת "רץ עכשיו". שאר המשרות ממתינות אחריה בתור.</p><div class="auto-apply-queue-list">${currentMarkup}${waitingMarkup}</div><div class="modal-actions"><button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`)}
