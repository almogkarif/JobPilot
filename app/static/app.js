...
function applicationProgressMarkup(){
  ...
  const verificationPending=status==='verification_pending',isRunning=status==='applying',queueSnapshot=normalizeAutoApplyQueue(data.auto_apply_queue||state.autoApplyQueue),runningQueueItem=queueSnapshot.running.find(item=>Number(item.id)===Number(data.application?.id)),duplicateRunning=isRunning&&Number(runningQueueItem?.duplicate_of||0)>0,failed=['failed','needs_input'].includes(status)&&!attentionWaiting,verified=status==='submitted'&&types.has('submission_verified');let firstPending=true;
...
function scheduleApplicationTrackingPoll(status=''){stopApplicationTrackingPoll();if(!trackedApplicationId)return;if(['submitted','verification_pending','failed','needs_input','cancelled'].includes(status))return;applicationTrackingTimer=setTimeout(pollApplicationTrackingStatus,applicationTrackingPollDelay(status))}
