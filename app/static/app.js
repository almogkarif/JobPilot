...
function autoQueueWaitingMarkup(item,index,runningCount){const position=Number(item.queue_position||index+1),duplicate=Number(item.duplicate_of||0),hasError=Boolean(String(item.last_error||'').trim()),tone=duplicate||hasError?'danger':position===1?'next':'waiting';const label=duplicate?'כפילות אפשרית':hasError?'הפעלה נכשלה':position===1?(runningCount?'הבאה בתור':'ממתינה ל־worker'):`מקום ${position} בתור`;const detail=duplicate?`נראית זהה להגשה #${duplicate} — מומלץ לבדוק לפני הפעלה`:hasError?String(item.last_error):runningCount?`ממתינה שאחד מ־${runningCount} ה־workers הפעילים יתפנה`:`ממתינה להפעלת worker ברקע`;return `<article class="auto-queue-waiting tone-${tone}"><b>${position}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ${esc(detail)}</small></span><em>${esc(label)}</em>${autoQueueRowActions(item)}</article>`}
function autoQueueAttentionMarkup(item){const blocker=item.blocker||{},options=blockerChoiceOptions(blocker),isQuestion=item.status==='needs_input'&&CHOICE_BLOCKER_KINDS.has(blocker.kind),isChoice=isQuestion&&options.length>0,tone=isQuestion?'question':'danger',headline=isQuestion?'מחכה לתשובה שלך':item.status==='verification_pending'?'השליחה טרם אומתה':'ההגשה נעצרה',question=blocker.question||blocker.field_label||'',detail=blocker.explanation||item.last_error||'נדרשת בדיקה לפני ניסיון נוסף',rediscover=blocker.legacy_choice_question?`<button class="btn secondary small" type="button" onclick="resolveBlockerAction(${blocker.id},'rediscover_question',${item.id})">השאלה לא מוצגת — זהה מחדש</button>`:'',interaction=isChoice?`<div class="auto-queue-inline-answer"><strong>${esc(question||'בחר תשובה')}</strong>${blockerChoiceControlMarkup(blocker,item.id)}${rediscover}</div>`:isQuestion?`<div class="auto-queue-inline-answer"><strong>${esc(question||'נדרשת תשובה')}</strong><div><input type="text" maxlength="255" data-text-blocker-input="${blocker.id}" value="${esc(applicationTextAnswerDrafts.get(Number(blocker.id))||'')}" placeholder="כתוב תשובה קצרה"><button class="btn primary small" type="button" data-text-blocker="${blocker.id}" data-text-application="${item.id}">שמור והמשך</button></div></div>`:'';return `<article class="auto-queue-attention tone-${tone}"><b>${isQuestion?'?':'!'}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ${esc(isQuestion?(question||detail):detail)}</small></span><em>${esc(headline)}</em>${interaction}${autoQueueRowActions(item)}</article>`}
