...
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),running=queue.running||[],waiting=queue.waiting||[],current=queue.current,total=Number(queue.total_active_count||running.length+waiting.length);if(!current&&!running.length&&!waiting.length){closeModal();toast('אין כרגע משרות בתור להגשה אוטומטית');return}const runningMarkup=running.map((item,index)=>autoQueueRunningMarkup(item,index,running.length)).join('');const queuedHead=!running.length&&current?.status==='queued'&&!waiting.some(item=>Number(item.id)===Number(current.id))?[current,...waiting]:waiting;const waitingMarkup=queuedHead.map((item,index)=>autoQueueWaitingMarkup(item,index,running.length)).join('');const runningSummary=running.length?`${running.length} ${running.length===1?'worker פעיל':'workers פעילים'} כרגע`:'אין worker פעיל כרגע';modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${total} ${total===1?'משרה פעילה':'משרות פעילות'} בתור</h2><p class="muted"><strong>${runningSummary}.</strong> ירוק מציין הגשה שרצה בפועל, צהוב מציין המתנה, ואדום מציין שגיאת הפעלה או כפילות אפשרית. המספור מתעדכן לפי סדר ההמתנה.</p><div class="auto-apply-queue-list">${runningMarkup}${waitingMarkup}</div><div class="modal-actions"><button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`)}
...
function openNotifications(){setMobileTabMenu(false);clearInterval(notificationTrackingRefreshTimer);refreshTrackingApplications().then(renderNotificationCenter);notificationTrackingRefreshTimer=setInterval(()=>refreshTrackingApplications().then(renderNotificationCenter),5000);renderNotificationCenter();$('#notification-center').classList.add('open');$('#notification-center').setAttribute('aria-hidden','false');$('#notification-trigger').setAttribute('aria-expanded','true')}
