...
async function showAutoApplyQueue(){const queue=await refreshAutoApplyQueue(),current=queue.current,waiting=queue.waiting||[];if(!current&&!waiting.length){toast('אין כרגע משרות בתור להגשה אוטומטית');return}const currentMarkup=current?`<article class="auto-queue-current ${current.status==='applying'?'is-running':'is-next'}"><b>${current.status==='applying'?'▶':Number(current.queue_position||1)}</b><span><strong>${esc(current.job?.title||'משרה')}</strong><small>${esc(current.job?.company||'')} · ${current.status==='applying'?'רץ עכשיו':'הבאה בתור'}</small></span><em>${current.status==='applying'?'רץ עכשיו':'הבאה'}</em></article>`:'';const waitingMarkup=waiting.map((item,index)=>`<article class="auto-queue-waiting"><b>${Number(item.queue_position||index+1)}</b><span><strong>${esc(item.job?.title||'משרה')}</strong><small>${esc(item.job?.company||'')} · ממתינה בתור</small></span><em>ממתינה</em></article>`).join('');modal(`<span class="kicker">תור הגשה אוטומטית</span><h2>${waiting.length?esc(autoQueueCountLabel(waiting.length)):'אין משרות נוספות שממתינות'}</h2><p class="muted">המשרה שמסומנת "רץ עכשיו" נשארת פעילה. הגשה אוטומטית חדשה נכנסת לראש רשימת ההמתנה, מיד אחריה.</p><div class="auto-apply-queue-list">${currentMarkup}${waitingMarkup}</div><div class="modal-actions"><button class="btn secondary" type="button" onclick="closeModal()">סגור</button></div>`)}
