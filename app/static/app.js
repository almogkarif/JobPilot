...
function applicationProgressMarkup(){
  ...
  const manualPanel=manualRequired?`<div class="application-live-warning"><strong>נדרשת הגשה ידנית</strong><small>${esc(blocker?.explanation||data.application?.agent_failure_detail||'מערכת הגיוס חסמה את האוטומציה. JobPilot לא ינסה שוב אוטומטית.')}</small><div class="card-actions"><a class="btn primary small" target="_blank" rel="noopener" href="${safeUrl(blockerTarget(blocker,data.application))}">פתח להגשה ידנית</a><button class="btn secondary small" type="button" onclick="markApplicationSubmitted(${data.application.id})">סמן כהוגש ידנית</button></div></div>`:'';
