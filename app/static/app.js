...
function trackingNavigatorMarkup(status=''){const {index,total}=trackedApplicationPosition(),known=index>=0;if(!known||!total)return '';const attemptCount=Number(applicationTrackingData?.application?.attempt_count||0),retryable=status==='failed'||(status==='queued'&&attemptCount>0);return `<nav class="application-tracker-navigator" aria-label="מעבר בין הגשות שלא הושלמו"><button type="button" onclick="moveTrackedApplication(-1)" aria-label="המשרה הקודמת" title="המשרה הקודמת" ${index===0?'disabled':''}>→</button><span><strong>משרה ${index+1} מתוך ${total}</strong><small>הגשות שטרם הושלמו</small></span>${retryable?`<button class="application-tracker-retry" type="button" onclick="retryTrackedApplication(${trackedApplicationId})" aria-label="הגשה מחדש" title="הגשה מחדש">↻</button>`:'<i></i>'}<button type="button" onclick="moveTrackedApplication(1)" aria-label="המשרה הבאה" title="המשרה הבאה" ${index===total-1?'disabled':''}>←</button></nav>`}
async function moveTrackedApplication(direction){await refreshTrackingApplications();const {index,total}=trackedApplicationPosition(),next=index+Number(direction||0);if(index<0||next<0||next>=total){renderNotificationCenter();return}startApplicationTracking(trackingApplications[next].id,false,true)}
...
function renderNotificationCenter() {
  ...
  if(liveTracker)liveTracker.insertAdjacentHTML('afterbegin',trackingNavigatorMarkup(applicationTrackingData?.application?.status||''));
  bindChoiceBlockerButtons(root);
