...
async function pollApplicationTrackingStatus(){if(!trackedApplicationId||applicationTrackingPollBusy)return;applicationTrackingPollBusy=true;try{const statusData=await api(`/api/applications/${trackedApplicationId}/tracking-status`);if(statusData.auto_apply_queue)setAutoApplyQueue(statusData.auto_apply_queue);const status=statusData.status||'',nextActiveId=Number(statusData.auto_apply_queue?.current?.id||0),version=String(statusData.timeline_version||'');if(['submitted','verification_pending','failed','needs_input'].includes(status)&&nextActiveId&&nextActiveId!==Number(trackedApplicationId)){startApplicationTracking(nextActiveId,false);return}if(!applicationTrackingData||!applicationTrackingVersion||version!==applicationTrackingVersion){await loadApplicationTimeline(version)}else{renderNotificationCenter()}if(status==='submitted'){clearTimeout(applicationTrackingAdvanceTimer);applicationTrackingAdvanceTimer=setTimeout(advanceTrackingToNextAutoQueue,2200)}scheduleApplicationTrackingPoll(status)}catch{stopApplicationTrackingPoll()}finally{applicationTrackingPollBusy=false}}
async function refreshApplicationTracking(){if(!trackedApplicationId)return;try{const statusData=await api(`/api/applications/${trackedApplicationId}/tracking-status`);if(statusData.auto_apply_queue)setAutoApplyQueue(statusData.auto_apply_queue);const version=String(statusData.timeline_version||'');await loadApplicationTimeline(version);const status=applicationTrackingData?.application?.status||statusData.status||'',nextActiveId=Number(statusData.auto_apply_queue?.current?.id||0);if(['verification_pending','failed','needs_input'].includes(status)&&nextActiveId&&nextActiveId!==Number(trackedApplicationId)){startApplicationTracking(nextActiveId,false);return}if(status==='submitted'){clearTimeout(applicationTrackingAdvanceTimer);applicationTrackingAdvanceTimer=setTimeout(advanceTrackingToNextAutoQueue,2200)}scheduleApplicationTrackingPoll(status)}catch{stopApplicationTrackingPoll()}}
