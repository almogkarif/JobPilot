...
async function copyApplicationFailureDiagnostics(){try{const payload=await api('/api/applications/failure-diagnostics'),failures=Array.isArray(payload.applications)?payload.applications:[],summary=payload.status_summary||{},header=[`JobPilot auto-apply diagnostics v5`,`generated_at: ${payload.generated_at||new Date().toISOString()} | career_track: ${payload.career_track||'—'} | incomplete: ${failures.length} | queued: ${Number(summary.queued_total||0)} | dispatch_sent: ${Number(summary.queued_dispatch_sent||0)} | needs_dispatch: ${Number(summary.queued_needs_dispatch||0)} | not_dispatchable: ${Number(summary.queued_not_dispatchable||0)} | stuck_queued: ${Number(summary.stuck_queued||0)} | stuck_applying: ${Number(summary.stuck_applying||0)} | manual_required: ${Number(summary.manual_required||0)} | verification_pending: ${Number(summary.verification_pending||0)}`,`profile_readiness: ${compactDiagnosticValue(payload.profile_readiness||{})}`].join('\n'),text=[header,...failures.map(applicationDiagnosticText)].join('\n\n');await navigator.clipboard.writeText(text);toast(`אבחון מפורט של ${failures.length} הגשות פעילות הועתק — כולל כל הממתינות בתור, מצב dispatch/worker, שאלות, שגיאות, ניסיונות ו־timeline`)}catch(error){toast(`העתקת האבחון נכשלה: ${error.message}`)}}
